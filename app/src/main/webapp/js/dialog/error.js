...
function showErrorDialog(exceptionName, stackTrace) {
    var some_html = '<h2 class="label label-danger label" style="display: block; font-size: xx-large">Виникла помилка</h2><br />';
    some_html += '<h4 class="center-block text-center">' + exceptionName + '</h4><br />';
    if(stackTrace !== undefined){
        some_html += '<button type="button" style="margin-bottom: 10px" class="btn btn-link btn-sm center-block" onclick="showStackTrace()">Показати деталі</button>';
